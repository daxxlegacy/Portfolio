...
export const Route = createFileRoute("/projects")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Selected projects: LCS Naval IT support, EPF API testing, business operations, and web development work." },
    ],
  }),
  component: Projects,
});
...
function Projects() {
  return (
    <div className="max-w-6xl mx-auto px-6 py-16">
      <h1 className="text-4xl sm:text-5xl font-bold">Featured <span className="bg-gradient-to-r from-primary to-primary-glow bg-clip-text text-transparent">Projects</span></h1>
      <p className="mt-4 text-muted-foreground max-w-2xl">Real work from professional roles — spanning IT infrastructure, software QA, and operational systems.</p>

      <div className="mt-12 grid md:grid-cols-2 gap-6">
        {projects.map((p) => (
          <article key={p.title} className="group relative rounded-2xl bg-card border border-border p-7 overflow-hidden hover:border-primary/50 transition-all shadow-[var(--shadow-card)]">
      ...
      <div className="mt-12 rounded-2xl border border-dashed border-border p-8 text-center">
        <p className="text-muted-foreground">More web development projects coming soon — built with Laravel, PHP, and MySQL.</p>
      </div>
    </div>
  );
}
